...
func sqlPrepare() {
	...
	var result1 = student{}
	stmt.QueryRow("E001", "1").Scan(&result1.name, &result1.grade)
...
func sqlExec() {
	...
	// exec with prepare statement
	stmt, err := db.Prepare("insert into tb_student values (?, ?, ?, ?)")
	stmt.Exec("N001", "Niko", 34, 2)
	fmt.Println("insert with prepare statement is success!")
	if err != nil {
		fmt.Println(err.Error())
		return
	}
